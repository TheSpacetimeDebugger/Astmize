...
Changes in v2.1.0:
  - /convert now accepts an optional "target_standard" field ("11" or "17", default "17")
    so the frontend can request C++11- or C++17-targeted output.
  - Health check now reports version 2.1.0.
...
def health_check():
    return jsonify({"status": "ok", "service": "Astmize API", "version": "2.1.0", "engine": "AI (OpenRouter)"})
...
def convert():
    ...
    if target_standard not in ("11", "17"):
        target_standard = "17"
    cpp_standard_label = f"C++{target_standard}"
